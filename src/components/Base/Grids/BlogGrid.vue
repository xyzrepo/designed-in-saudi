<template>
    <div class="bg-white px-4 pt-16 pb-20 sm:px-6 lg:px-8 lg:pt-24 lg:pb-28">
        <div
            ...
            <div>
                <!-- <PrimaryTitle size="5xl" color="gray-900" accent="primary-900" gradient>{{title}}</PrimaryTitle> -->
                <div class="title gradient">{{ title }}</div>
                <p class="mt-3 text-xl text-gray-500 sm:mt-4">{{ subtitle }}</p>
            </div>
            <div
                class="mt-12 grid gap-16 pt-12 lg:grid-cols-3 lg:gap-x-5 lg:gap-y-12">
                <BlogCard v-for="post in posts" :key="post.title" :item="post">
                </BlogCard>
            </div>
        </div>
    </div>
...
<script setup>
defineProps({
    title: { type: String, default: 'Recent publications' },
    subtitle: {
        type: String,
        default:
