...
<template>
    <RadioGroup v-model="selectedOption" class="my-6">
        ...
        <div :class="`mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-${grid ? '2' : '1'} lg:grid-cols-${grid ? '3' : '1'} sm:gap-x-4`" v-show="selectedOption === null">
            <RadioGroupOption as="span" v-for="(option, i) in options" :key="option.id" :value="option" v-slot="{ checked, active }" 
                @click="$emit('update:modelValue', selectedOption);"
                :tabindex="i">
                <div :class="[focus === i ? 'bg-red-700' : '', checked ? 'border-transparent' : 'border-gray-300', active ? 'border-indigo-500 ring-2 ring-indigo-500' : '', 'relative bg-white border rounded-lg shadow-sm p-4 flex cursor-pointer focus:outline-none']" >
