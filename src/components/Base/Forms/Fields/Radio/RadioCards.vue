...
<template>
    <RadioGroup v-model="selectedOption" class="my-10">
        <RadioGroupLabel class="pt-8 text-2xl font-medium text-gray-900">{{label}}</RadioGroupLabel>

        <div class="mt-1 pt-4 flex text-2xl text-gray-900 sm:mt-0 sm:col-span-2" v-if="selectedOption">
          ...
          <span class="ml-4 flex-shrink-0">
            ...
            <SecondaryButton @click="selectedOption = null">
                <RefreshIcon class="w-5 h-5 stroke-2"/>
                Update
            </SecondaryButton>
          </span>
        </div>

        <div class="mt-4 grid grid-cols-1 gap-y-6 sm:grid-cols-1 lg:grid-cols-1 sm:gap-x-4" v-show="!selectedOption">
