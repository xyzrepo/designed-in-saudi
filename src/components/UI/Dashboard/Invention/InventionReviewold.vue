        ...
        <div class="mt-5 border-t border-gray-200">
            <dl class="sm:divide-y sm:divide-gray-200">
                <div
                    ...
                    <span
                        v-if="item === 'features' && !!data.features"
                        role="list"
                        class="divide-y divide-gray-200">
                        <li
                            v-for="(feature, i) in data.features"
                            :key="i"
                            class="flex flex-row py-1 hover:text-primary-700">
                            <!-- <img class="h-10 w-10 rounded-full" :src="person.image" alt="" /> -->
                            <!-- <div class="ml-3 w-full relative"> -->
                            <span
                                class="text-md block font-medium text-gray-700">
                                {{ feature.name }} ::
                            </span>
                            <span
                                class="text-md inline-block py-4 pr-4 font-light text-gray-700">{{ feature.value }}</span>
                            <!-- </div> -->
                        </li>
                    </span>
